...
# Phase 3 (Task-Specific): Gated Residual Corrector Training Orchestrator
# Trains 10 individual correctors (one per task) across 3 seeds.

RESUME="${RESUME:-1}"
HEARTBEAT_SEC="${HEARTBEAT_SEC:-60}"
WANDB_PROJECT="${WANDB_PROJECT:-gated_residual_phase3_task_specific}"
...
OUTPUT_ROOT="${OUTPUT_ROOT:-$SCRIPT_DIR/outputs/phase3_task_specific_train_$(date +%Y%m%d_%H%M%S)}"
