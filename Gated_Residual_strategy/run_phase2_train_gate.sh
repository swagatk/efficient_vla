...
# Phase 2: Failure-Risk Gate Training Orchestrator
# Adheres to the create_new_script skill for robust, resumable jobs.

RESUME="${RESUME:-1}"
HEARTBEAT_SEC="${HEARTBEAT_SEC:-60}"
WANDB_PROJECT="${WANDB_PROJECT:-gated_residual_phase2}"
...
OUTPUT_ROOT="${OUTPUT_ROOT:-$SCRIPT_DIR/outputs/phase2_train_$(date +%Y%m%d_%H%M%S)}"
