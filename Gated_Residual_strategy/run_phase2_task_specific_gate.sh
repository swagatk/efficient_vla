...
# Phase 2 (Task-Specific): Failure-Risk Gate Training Orchestrator
# Trains 10 individual gates (one per task) across 3 seeds.

RESUME="${RESUME:-1}"
HEARTBEAT_SEC="${HEARTBEAT_SEC:-60}"
WANDB_PROJECT="${WANDB_PROJECT:-gated_residual_phase2_task_specific}"
...
for seed in "${SEEDS[@]}"; do
  for task_id in "${TASKS[@]}"; do
    ...
    UNIT="task_${task_id}_seed_${seed}"
    UNIT_DIR="$OUTPUT_ROOT/task_${task_id}/unit_train_seed_${seed}"
    ...
    CMD=(
      env PYTHONUNBUFFERED=1 WANDB_RUN_ID_FILE="$RUN_ID_FILE" "$PYTHON_BIN" "-u" "$SCRIPT_DIR/train_failure_risk_gate.py"
      --data_dir "$DATA_DIR"
      --output_dir "$UNIT_DIR"
      --task_id "$task_id"
      --seed "$seed"
