...
# Week 3: Stage-1 Performance Sweep (GroundingDINO Text Hints)
#
# What this script does:
# 1) Runs 3 seeded evaluations using GroundingDINO for text hints.
...
# 4) Produces aggregate Week-3 summary reports.
#
# Usage:
...
# Sweep Arrays
GROUNDING_FREQUENCIES="${GROUNDING_FREQUENCIES:-1 5}"
BOX_STYLES="${BOX_STYLES:-edge filled mask}"
...
RUN_NAME="week3_stage1_sweep_${TIMESTAMP}"
OUTPUT_ROOT="${OUTPUT_ROOT:-$HOME/lerobot/outputs/week3_stage1_sweep/${RUN_NAME}}"
mkdir -p "$OUTPUT_ROOT"
...
# Still leveraging the Week 2 python eval script as the core logic is identical
WEEK3_ENTRYPOINT="$SCRIPT_DIR/eval_week2_visual_prompting.py"
if [[ ! -f "$WEEK3_ENTRYPOINT" ]]; then
  echo "ERROR: missing $WEEK3_ENTRYPOINT" >&2
  exit 1
fi
...
SUMMARY_CSV="$CONSOLIDATED_DIR/week3_runs_summary.csv"
TASK_SUMMARY_CSV="$CONSOLIDATED_DIR/week3_task_success_summary.csv"
AGGREGATE_REPORT="$CONSOLIDATED_DIR/week3_aggregate_report.txt"
...
echo "Starting Week-3 Stage 1 Performance Sweep runs (GroundingDINO)..."
...
for FREQ in $GROUNDING_FREQUENCIES; do
  for STYLE in $BOX_STYLES; do
    echo ""
    echo "=========================================================="
    echo "Sweep Config: Freq=$FREQ | Style=$STYLE"
    echo "=========================================================="
    
    for SEED in $SEEDS; do
      RUN_DIR="$OUTPUT_ROOT/freq_${FREQ}_style_${STYLE}/seed_${SEED}"
      ...
      CMD=(
        "$default_python" "$WEEK3_ENTRYPOINT"
        "--model_id=$MODEL_ID"
      ...
      PARSED_FIELDS="$($default_python - "$EVAL_INFO_JSON" "$SEED" "$TASK_SUMMARY_CSV" "$FREQ" "$STYLE" <<'PY'
import json
import pathlib
import sys
...
PY
      ...
      echo "$SEED,$FREQ,$STYLE,$SUCCESS_RATE,$PEAK_VRAM_GB,$AVG_STEP_LATENCY_MS,$E2E_LOOP_RATE_HZ,$WALL_SEC,$EXIT_CODE,$RUN_DIR,$EVAL_S,$EVAL_EP_S,$TASK_SUCCESS_RATES" >> "$SUMMARY_CSV"
...
$default_python - "$SUMMARY_CSV" "$TARGET_SUCCESS" <<'PY' > "$AGGREGATE_REPORT"
import csv
import statistics
import sys
...
with open(summary_csv, newline="") as f:
    ...
    for r in reader:
        cfg = (r['grounding_freq'], r['box_style'])
        runs.setdefault(cfg, []).append(r)

print("Week 3 Stage-1 Sweep: Aggregate Report")
print("=" * 44)
...
for cfg, rows in runs.items():
    ...
    for r in rows:
        try:
            success_vals.append(float(r['success_rate']))
        except Exception:
            pass
        try:
            latency_vals.append(float(r['avg_step_latency_ms']))
        except Exception:
            pass
            
    mean_succ = statistics.fmean(success_vals) if success_vals else 0.0
    std_succ = statistics.stdev(success_vals) if len(success_vals) > 1 else 0.0
    mean_lat = statistics.fmean(latency_vals) if latency_vals else 0.0
    
    print(f"-> Freq: {cfg[0]:>2} | Style: {cfg[1]:>8} | Mean Success: {mean_succ:5.2f}% (std {std_succ:4.2f}) | Mean Latency: {mean_lat:6.2f}ms")
PY

echo ""
echo "Week-3 Stage 1 sweep 3-seed run complete."
